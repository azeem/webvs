import IMain from "../../../src/IMain";
import { mainTest } from "../funcTestUtils";

describe("Picture", () => {
    it("should render image from resource manager", () => {
        return mainTest({
            expectImageSrc: "Picture_0.png",
            onInit: (main: IMain) => {
                // tslint:disable-next-line:max-line-length
                main.getRsrcMan().registerUri("test_image_1.png", "data:image/png;base64,");
            },
            preset: {
                components: [
                    {
                        src: "test_image_1.png",
                        type: "Picture",
                        x: -0.7, y: -0.7,
                    },
                ],
            },
        });
    });

    it("should render image from builtin resource pack #1", () => {
        return mainTest({
            expectImageSrc: "Picture_1.png",
            preset: {
                components: [
                    {
                        src: "avsres_texer_square_edgeonly_30x30.bmp",
                        type: "Picture",
                        x: 0, y: 0,
                    },
                ],
            },
        });
    });

    it("should render image from builtin resource pack #2", () => {
        return mainTest({
            expectImageSrc: "Picture_2.png",
            preset: {
                components: [
                    {
                        src: "avsres_texer_circle_edgeonly_19x19.bmp",
                        type: "Picture",
                        x: -1, y: 0.5,
                    },
                ],
            },
        });
    });
});
