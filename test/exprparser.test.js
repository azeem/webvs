...
test('ExprParser GLSL Generation', function() {
    ...
    var glslExpect = "float x;float y;float r;float d;uniform float b;uniform float w;uniform float h;uniform float cid;uniform float c;uniform float f;uniform float dt;uniform float dl;uniform float beatdiv;uniform float t;uniform float dx;uniform float dy;uniform vec2 __randStep;\nvec2 __randSeed;\nfloat rand(float max) {\n   __randCur += __randStep;\n   float val = fract(sin(dot(__randSeed.xy ,vec2(12.9898,78.233))) * 43758.5453);\n   return (floor(val*max)+1);\n}void perPixel() {x=(x+((sin((y*dx)))*0.03));\ny=((rand(y))-((cos((x*dy)))*0.03));}";
