...
function testPicture(canvas, gl, fm, copier, name, src, value) {
    var pic = new Webvs.Picture({
        src: src,
        x: -0.7, y: -0.7
    });
    pic.init(gl, new DummyMain(canvas), new DummyParent(fm));
    ...
    equal(
        canvas.toDataURL(),
        value,
        "Picture image in test "+name+" should be correct"
    );
}

CanvasTestWithFM("Picture test", 1, {async: false}, function(canvas, gl, fm, copier) {
    return testPicture(
        canvas, gl, fm, copier,
        "Picture2", 
